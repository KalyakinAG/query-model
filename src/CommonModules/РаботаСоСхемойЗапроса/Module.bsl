...
Функция ЗапросПакетаИзТаблицы(СхемаЗапроса, Знач ТаблицаИсточника, Знач ПсевдонимИсточника = "", Знач ВыбираемыеКолонки = Неопределено, ТаблицаДляПомещения = "", ЗапросНаУничтожение = Ложь, ВыбиратьРазрешенные = Ложь, ОператорВыбрать = Неопределено, Очистить = Ложь) Экспорт
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		ЗапросПакета 	= РаботаСоСхемойЗапроса.ЗапросПакета(СхемаЗапроса, ТаблицаДляПомещения);
		ОператорВыбрать = РаботаСоСхемойЗапроса.Оператор(ЗапросПакета);
		Источник = РаботаСоСхемойЗапроса.Источник(ОператорВыбрать, ТаблицаИсточника, ПсевдонимИсточника);
		...
		//		Продолжить;
		//	КонецЕсли;
		//	РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, ?(ПустаяСтрока(ПсевдонимИсточника), "", ПсевдонимИсточника+".")+Поле.Имя, Поле.Имя);
		//КонецЦикла;
	Иначе
		...
		Если ПустаяСтрока(ПсевдонимИсточника) Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ЗапросПакета 	= РаботаСоСхемойЗапроса.ДобавитьЗапросПакета(СхемаЗапроса, ТаблицаДляПомещения);
		...
		//		Продолжить;
		//	КонецЕсли;
		//	РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, ПсевдонимИсточника+"."+Поле.Псевдоним, Поле.Псевдоним);
		//КонецЦикла;
	КонецЕсли;
	...
	Если ВыбираемыеКолонки <> Неопределено Тогда
		...
		Иначе
			...
			КонецЦикла;
		КонецЕсли;
		//Для каждого ПутьКПолю Из ОбщийКлиентСервер.Массив(ВыбираемыеКолонки) Цикл
...
Функция ОписаниеВременнойТаблицы(ИмяТаблицы, ИсточникДанных = Неопределено, КоллекцияКолонок = Неопределено) Экспорт
	Если ТипЗнч(КоллекцияКолонок) = Тип("Строка") Тогда
		мКолонки = ОбщийКлиентСервер.Массив(КоллекцияКолонок);
	Иначе
	...
	ИначеЕсли ТипЗначенияИсточника = Тип("ДанныеФормыКоллекция") Тогда
		ДоступныеПоля = ДоступныеПоля(ИсточникДанных.Выгрузить(Новый Массив).Колонки, мКолонки, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ИсходныйНомерСтроки"));
...
//					- Структура - Структуру можно получить из сервисных функций (см. функцию РаботаСоСхемойЗапроса.ПолучитьОписаниеВложенногоЗапроса(), РаботаСоСхемойЗапроса.ПолучитьОписаниеВременнойТаблицы(ИмяТаблицы)).
...
// Примеры:
//  Новое поле:
//		РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "Документ.Контрагент", "Контрагент");
// 	Установка поля или подмена в копии оператора, можно подменить путь или выражение для колонки запроса:
//		РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)", "Контрагент");
//
...
//		РаботаСоСхемойЗапроса.ВыражениеПоля(ОператорВыбрать, "СУММА", "Документ.СуммаДокумента");
// 	Пример для подмены, можно подменить на существующее поле или выражение:
//		РаботаСоСхемойЗапроса.ВыражениеПоля(ОператорВыбрать,, "Документ.Контрагент", "ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)");
//		РаботаСоСхемойЗапроса.ВыражениеПоля(ОператорВыбрать,, "Документ.Контрагент", "Документ.Организация");
...
//
// Пример:
//  РаботаСоСхемойЗапроса.ИзменитьПсевдонимКолонки(ЗапросПакета, "СуммаОстаток", "СуммаРазмещено");
//
...
Функция УсловиеВхождения(Источник, ИсточникДанных = Неопределено, Поля = "*") Экспорт
	...
	КонецЦикла;
	Если ЗначениеЗаполнено(Источник) Тогда
		ЗапросПакета = РаботаСоСхемойЗапроса.ЗапросПакетаИзТаблицы(СхемаЗапроса, РаботаСоСхемойЗапроса.ОписаниеВременнойТаблицы(Источник, ИсточникДанных, МассивПолейСправа),, МассивПолейСправа,,,, ОператорВыбрать);
	...
	Иначе
		...
		Для Индекс = 0 По МассивПолейСлева.ВГраница() Цикл
			РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, МассивПолейСправа[Индекс], МассивПолейСлева[Индекс]);
		КонецЦикла;
	КонецЕсли;
...
//
// Пример:
//	РаботаСоСхемойЗапроса.ДобавитьСоединениеХарактеристики(ОператорВыбрать, "ВТ_ДВИЖЕНИЯ_ОПЕРАЦИИ", "СтатьяИсточникАналитика1",
...
Процедура ДобавитьСоединениеХарактеристики(Оператор, Знач ИсточникОбъекта, Знач ИсточникХарактеристики, ПолеОбъектаИсточника = "Ссылка", 
	...
	РаботаСоСхемойЗапроса.ДобавитьСоединение(,
		ИсточникОбъекта,
...
Процедура ДобавитьУсловиеХарактеристики(Оператор, Источник, ИсточникЗначений, ПолеОбъектаИсточника = "Ссылка", ПолеОбъекта = "Объект", ПолеВида = "Свойство", ПолеЗначения = "Значение", ВыражениеПараметраВида = "", ВыражениеПараметраЗначения = "") Экспорт 
	...
	РаботаСоСхемойЗапроса.ДобавитьИсточник(Оператор, 
		ИсточникЗначений,
...
//
// Пример:
//	РаботаСоСхемойЗапроса.ДобавитьУсловиеОграниченияПоВидуДоступа(СхемаЗапроса, ОператорИсточника,, "ЦФО");
...
Процедура ДобавитьУсловиеОграниченияПоВидуДоступа(СхемаЗапроса, ОператорИсточника, ПсевдонимИсточника = "Источник", ВидДоступа) Экспорт
	////////////////////////////////////////////////////////////////////////////////
	//  ЗАПРОС ПАКЕТА 0
	ЗапросПакета 	= РаботаСоСхемойЗапроса.ДобавитьЗапросПакета(СхемаЗапроса, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА");
	ОператорВыбрать = РаботаСоСхемойЗапроса.ДобавитьОператор(ЗапросПакета, Новый Структура("ВыбиратьРазличные", Истина));
	РаботаСоСхемойЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.Пользователи"					, "СправочникПользователи");
	РаботаСоСхемойЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.ГруппыДоступа");
	РаботаСоСхемойЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.ГруппыДоступа.ЗначенияДоступа"	, "ГруппыДоступаЗначенияДоступа");
	РаботаСоСхемойЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.ГруппыДоступа.ВидыДоступа"		, "ГруппыДоступаВидыДоступа");
	РаботаСоСхемойЗапроса.ДобавитьСоединение(ОператорВыбрать, "СправочникПользователи"		, "ГруппыДоступа"				, "СправочникПользователи.Ссылка = ГруппыДоступа.Пользователь", ТипСоединенияСхемыЗапроса.Внутреннее);
	РаботаСоСхемойЗапроса.ДобавитьСоединение(ОператорВыбрать, "ГруппыДоступа"				, "ГруппыДоступаЗначенияДоступа", "ГруппыДоступа.Ссылка = ГруппыДоступаЗначенияДоступа.Ссылка", ТипСоединенияСхемыЗапроса.Внутреннее);
	РаботаСоСхемойЗапроса.ДобавитьСоединение(ОператорВыбрать, "ГруппыДоступаЗначенияДоступа", "ГруппыДоступаВидыДоступа"	, 
	...
	РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ГруппыДоступаЗначенияДоступа.ВидДоступа");
	РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ГруппыДоступаВидыДоступа.ВсеРазрешены");
	РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ГруппыДоступаЗначенияДоступа.ЗначениеДоступа");
	//  Переместить запрос пакета в начало
	РаботаСоСхемойЗапроса.УстановитьИндексЗапросаПакета(СхемаЗапроса, ЗапросПакета, 0);
	////////////////////////////////////////////////////////////////////////////////
	//  ЗАПРОС ПАКЕТА 1
	РаботаСоСхемойЗапроса.ДобавитьИсточник(ОператорИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА", "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ");
	РаботаСоСхемойЗапроса.ДобавитьИсточник(ОператорИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА", "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ");
	РаботаСоСхемойЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ", ПсевдонимИсточника+".Ссылка = ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ.ЗначениеДоступа");
	РаботаСоСхемойЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ", "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ.ВсеРазрешены");
	РаботаСоСхемойЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ" , ПсевдонимИсточника+".Ссылка = ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ.ЗначениеДоступа");
	РаботаСоСхемойЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ" , "НЕ ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ.ВсеРазрешены");
...
//		РаботаСоСхемойЗапроса.Итог(ЗапросПакета, "Контрагент");
//		ЗапросПакета.ВыраженияИтогов.Добавить("СУММА(СуммаУпр)", "СуммаУпр");
//	или можно через выражение:
//		РаботаСоСхемойЗапроса.Итог(ЗапросПакета, "Контрагент.ИНН+Контрагент.КПП", "ИННКПП");
//		ЗапросПакета.ВыраженияИтогов.Добавить("СУММА(СуммаУпр)", "СуммаУпр");
//
// Параметры:
//  ЗапросПакета - ЗапросСхемыЗапроса
...
Функция ДобавитьЗапросПакетаИзТаблицы(СхемаЗапроса, Знач ТаблицаИсточника, Знач ПсевдонимИсточника = "", Знач ВыбираемыеКолонки = Неопределено, ТаблицаДляПомещения = "", ЗапросНаУничтожение = Ложь, ВыбиратьРазрешенные = Ложь, ОператорВыбрать = Неопределено) Экспорт
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		ЗапросПакета 	= РаботаСоСхемойЗапроса.ДобавитьЗапросПакета(СхемаЗапроса, ТаблицаДляПомещения);
		ОператорВыбрать = РаботаСоСхемойЗапроса.ДобавитьОператор(ЗапросПакета);
		РаботаСоСхемойЗапроса.ДобавитьИсточник(ОператорВыбрать, ТаблицаИсточника, ПсевдонимИсточника);
		Для каждого Поле Из ТаблицаИсточника.ДоступныеПоля Цикл
			РаботаСоСхемойЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, ?(ПустаяСтрока(ПсевдонимИсточника), "", ПсевдонимИсточника+".")+Поле.Имя, Поле.Имя);
		КонецЦикла;
	Иначе
