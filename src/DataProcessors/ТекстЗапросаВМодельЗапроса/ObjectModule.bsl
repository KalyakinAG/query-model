...
Процедура ДобавитьТекстПоля(ЗапросПакета, Выражение)
	...
	Если СтрНачинаетсяС(Поле, "ЕСТЬNULL(") Тогда
		...
		Если Колонка.Поля.Количество() = 1 И Псевдоним = Колонка.Псевдоним Тогда
			МодельТекста
				.Добавить(".Поле(""%1"", , ""%2"")", ОбщийКлиентСервер.ЭкранироватьТекст(Поле), ОбщийКлиентСервер.ЭкранироватьТекст(ПолеЕстьNull))
			;
			Возврат;
		КонецЕсли;
		МодельТекста
...
Процедура ТекстЗапросаПакета(ЗапросПакета)
	...
	Для Каждого ВыражениеИтогов Из ЗапросПакета.ВыраженияИтогов Цикл
		...
		Иначе
			МодельТекста
				.Добавить(".Итог(""%1"", ""%2"")", ОбщийКлиентСервер.ЭкранироватьТекст(ВыражениеИтогов.Выражение), ВыражениеИтогов.Поле.Псевдоним)
			;
		КонецЕсли;
	КонецЦикла;
