...
Функция Объединить(ИсходныйОператор = Неопределено, ТипОбъединения = Неопределено) Экспорт
	Если ИсходныйОператор = Неопределено Тогда
		ОператорВыбрать = РаботаСоСхемойЗапроса.Оператор(ЗапросПакета);
	Иначе
		ОператорВыбрать = РаботаСоСхемойЗапроса.КопияОператора(ЗапросПакета, ИсходныйОператор);
	КонецЕсли;	
	ОператорВыбрать.ТипОбъединения = ?(ТипОбъединения = Неопределено, ТипОбъединенияСхемыЗапроса.Объединить, ТипОбъединения);
	Возврат ЭтотОбъект;
КонецФункции

Функция ОбъединитьВсе(ИсходныйОператор = Неопределено) Экспорт
	Возврат Объединить(ИсходныйОператор, ТипОбъединенияСхемыЗапроса.ОбъединитьВсе);
КонецФункции
...
Функция Источник(Таблица, Псевдоним = "", ПараметрыТаблицы = Неопределено) Экспорт
	...
	Если ТипТаблицы = Тип("Строка") Тогда
		...
		Возврат ЭтотОбъект;
	КонецЕсли;
	ВременнаяТаблица = РаботаСоСхемойЗапроса.ОписаниеВременнойТаблицы(ПараметрыТаблицы, Таблица);
	Источник = РаботаСоСхемойЗапроса.Источник(ОператорВыбрать, ВременнаяТаблица, Псевдоним);
	ИсточникСлева = Источник;
	Возврат ЭтотОбъект;
КонецФункции
...
Функция Условие(Выражение) Экспорт
	...
	Если ПараметрыТаблицы.Свойство("Условие", ТекущееВыражение) Тогда
...
Функция УсловиеСвязиВхождения(ИмяТаблицы, Поля) Экспорт
