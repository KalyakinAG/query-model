...
Процедура ДобавитьТекстПоля(ЗапросПакета, Выражение)
	...
	Если СтрНачинаетсяС(Поле, "ЕСТЬNULL(") Тогда
		ЧислоСимволов = 9;
		ПозицияЗакрывающейсяСкобки = СтрНайти(Поле, ")", НаправлениеПоиска.СКонца);
		Поле = Сред(Поле, ЧислоСимволов + 1, ПозицияЗакрывающейсяСкобки - 1 - ЧислоСимволов);
		...
		Если Колонка.Поля.Количество() = 1 И Псевдоним = Колонка.Псевдоним Тогда
			МодельТекста
				.Добавить("РаботаСМодельюЗапроса.Поле(МодельЗапроса, ""%1"", , ""%2"");", ОбщийКлиентСервер.ЭкранироватьТекст(Поле), ОбщийКлиентСервер.ЭкранироватьТекст(ПолеЕстьNull))
			;
			Возврат;
		КонецЕсли;
		МодельТекста
...
Процедура ТекстЗапросаПакета(ЗапросПакета)
	...
	Для Каждого ВыражениеИтогов Из ЗапросПакета.ВыраженияИтогов Цикл
		...
		Иначе
			МодельТекста
				.Добавить("РаботаСМодельюЗапроса.Итог(МодельЗапроса, ""%1"", ""%2"");", ОбщийКлиентСервер.ЭкранироватьТекст(ВыражениеИтогов.Выражение), ВыражениеИтогов.Поле.Псевдоним)
			;
		КонецЕсли;
	КонецЦикла;
